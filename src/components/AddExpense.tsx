...
export default function AddExpense({ onAdd, participants }: AddExpenseProps) {
  ...
  const handleExpenseDetected = (detectedExpense: Partial<Expense>) => {
    if (detectedExpense.description) setDescription(detectedExpense.description);
    if (detectedExpense.amount) setAmount(detectedExpense.amount.toString());
    if (detectedExpense.paidBy) setPaidBy(detectedExpense.paidBy);
    if (detectedExpense.splitAmong) setSplitAmong(detectedExpense.splitAmong);
    setShowReceiptUpload(false);
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        ...
        >
          {showReceiptUpload ? 'Hide Receipt Upload' : 'Upload Receipt'}
        </button>
      </div>
      ...
      {showReceiptUpload && (
        <div className="mb-4">
          <ReceiptUpload
            ...
            paidById={paidBy || (participants[0]?.id || '')}
          />
        </div>
      )}
